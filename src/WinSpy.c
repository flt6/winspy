...
HWND CreateTooltip(HWND hwndDlg)
{
	...
	} CtrlTips[] = 
	{
		...
		-1, IDC_HIDDEN,		_T("显示隐藏的窗口"),
		-1, IDC_CAPTURE,    _T("捕获当前窗口 (Alt+C)"),
		-1, IDOK,			_T("Exit WinSpy++"),
		-1, IDC_EXPAND,     _T("Expand / Collapse (F3)"),
		-1, IDC_REFRESH,    _T("刷新窗口列表 (F6)"),
		-1, IDC_LOCATE,     _T("定位当前窗口"),
		-1, IDC_FLASH,      _T("高亮选中窗口"),

		GENERAL_TAB, IDC_HANDLE_MENU,  _T("Window Commands"),
		GENERAL_TAB, IDC_SETCAPTION,   _T("修改窗口标题"),
		GENERAL_TAB, IDC_EDITSIZE,     _T("调整窗口位置"),
		STYLE_TAB,	 IDC_EDITSTYLE,    _T("修改风格"),
		STYLE_TAB,   IDC_EDITSTYLEEX,  _T("修改拓展风格"),
		PROCESS_TAB, IDC_PROCESS_MENU, _T("Process Commands"),
		WINDOW_TAB,  IDC_PARENT,       _T("父窗口"),
